...
def search_index(whoosh_index, search_fields, query):
    ...
    for ix in range(SEARCH_TRIES):
        try:
            ...
            log.warning("Failed 'search_index' with an EmptyIndexError.")
            time.sleep(SEARCH_PAUSE)

            if ix == SEARCH_TRIES-1:
                raise Exception("Failed 'search_index' too many times.")
